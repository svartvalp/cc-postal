          ...
          <v-list-item-group>
            <template v-for="(item,index) in departures">
              ...
              <v-list-item :key="item.id" @click="redirectToDeparture(item)">
                <v-list-item-content>
                  <v-list-item-title class="addresses">Адрес доставки: <span>{{item.arrivingPoint.address}}</span></v-list-item-title>
                </v-list-item-content>
                <v-list-item-content class="middle-content">
                  <v-list-item-title class="departure-date">Дата отправления: <span>{{item.departureDate}}</span></v-list-item-title>
                  <v-list-item-title class="departure-type">Тип посылки: <span>{{item.type}}</span></v-list-item-title>
                </v-list-item-content>
                <v-list-item-content class="text-right">
                  <v-list-item-title class="addresses"><span v-if="item.arrived">Доставлено</span><span v-else-if="!item.arrived">В пути</span></v-list-item-title>
                </v-list-item-content>
              </v-list-item>
            </template>
          </v-list-item-group>
